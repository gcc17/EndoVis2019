...
from utils import get_test_cases, get_train_case, get_test_gt, get_train_gt, name2num
...
class TestDataset(Dataset):
    def __init__(self, feature_name, feature_type='rgb', length=512):
        ...
        self.length = length
        self.test_cases, self.clip_nums = get_test_cases(feature_name,
                                                          feature_type, length)
    ...
    def __getitem__(self, idx):
        return_dict = {}
        return_dict['idx'] = np.array(name2num(self.feature_name, idx))
        return_dict['gt'] = get_test_gt(self.feature_name[idx],
                                        self.clip_nums[idx], self.length)
        return_dict['data'] = np.array(self.test_cases[idx])
        return_dict['is_test_case'] = 1
        return_dict['clip_num'] = self.clip_nums[idx]
...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return_dict['gt_phase'], return_dict['gt_instrument'], return_dict['gt_action'] \
            = get_train_gt(name, frame*i3d_time, self.length)
